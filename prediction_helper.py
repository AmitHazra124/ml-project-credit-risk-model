...
def prepare_df(age, income, loan_amount, loan_tenure_months, avg_dpd_per_delinquency,
    ...
    input_data = {
        ...
        # dummy values for columns the scaler expects but the final model doesn't use
...
def calculate_credit_score(input_df, best_score = 300, scale_length = 600):
    x = np.dot(input_df[features].values, model.coef_.T) + model.intercept_

    default_probability = 1 / (1 + np.exp(-x))
    non_default_probability = 1 - default_probability

    credit_score = best_score + non_default_probability.flatten()[0] * scale_length
